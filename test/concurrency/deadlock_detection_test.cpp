...
TEST(LockManagerDeadlockDetectionTest, BasicDeadlockDetectionTest1) {
  ...
  std::thread t2([&] {
    ...
    // This will block
    res = lock_mgr.LockRow(txn2, LockManager::LockMode::EXCLUSIVE, toid, rid3);
    EXPECT_EQ(true, res);

    lock_mgr.UnlockRow(txn2, toid, rid3);
  ...
  std::thread t3([&] {
    ...
    // This will block
    res = lock_mgr.LockRow(txn3, LockManager::LockMode::EXCLUSIVE, toid, rid2);
    EXPECT_EQ(res, false);
